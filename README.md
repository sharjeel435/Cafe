...
The sample/demo catalog contains 33 curated dishes with local photos across BBQ, karahi and handi, vegetarian meals, rice, Chinese dishes, soups, sandwiches, pasta, seafood, snacks, breads, desserts, and drinks. Repetitive rice, burger, and tea variants have been removed. Menu photos are sourced from the Kababjees Highway listing on Foodpanda; individual source URLs are recorded in `public/food/SOURCES.json`. Prices remain illustrative demo prices, with no restaurant affiliation or ordering integration. Existing sessions refresh dish details and photos, preserve stock changes, balances and order history, and remove retired dishes from carts.
